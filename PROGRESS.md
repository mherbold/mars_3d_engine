...
## Current Focus
**Agents:** This section is for recording the current focus of development, whether it's a specific milestone, feature, or bug that is being actively worked on. Update this section with a brief description of the current focus, along with any relevant context or links to discussions.
...
| M6 | DLSS 4 integration (SR, RR, MFG) | ✅ | AOV textures black until M7/M8 shaders write real G-buffer data |
...
- 🔲 Update AOV writes in closest-hit HLSL to account for multi-bounce GI radiance contributions
...
| Camera system (`camera.h` / `camera.cpp`) is scaffolded but not yet implemented; full `Camera` class (view/proj matrices, motion vectors, jitter) is part of M5. |
